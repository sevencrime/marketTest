...
class MarketRocksDBClient(RocksDBClient):
    ...
    def get_today_close_price(self, exchange_int, instr_code, close_time):
        # 期货市场收市，期货的结算价却未给出，则取其收盘价
        strptimeFunc = lambda x, fmt="%Y%m%d%H%M%S":  datetime.datetime.strptime(x, fmt)    # str 转 detetime
        strftimeFunc = lambda x, fmt="%Y%m%d%H%M%S": datetime.datetime.strftime(x, fmt)  # datetime 转 str
        min_value = None
        start = time.time()
        while not min_value:
            ...
            min_key = 'KLINEMIN_{}_{}_{}'.format(exchange_int, instr_code, close_time)
            min_value = self.get_single_value(min_key)
            close_time = strftimeFunc(strptimeFunc(close_time) - datetime.timedelta(minutes=1))

            if min_value:
                self.logger.debug("rocksdb获取到的数据 {} 时刻的数据 : {}".format(close_time, min_value))
...
if __name__ == '__main__':
    db = MarketRocksDBClient(stock_rocksdb_path)
    db_method = DBMethod()
    common = Common()
    # key_list = ['KLINE_16_HHI2006_10_20200615160000', 'KLINEMIN_16_HHI2006_20200615160000', 'KLINEMIN_16_HHI2007_20200617112900']
    # time_start = 20200630171500
    # # time_end = 20200701183000
    # time_end = 20200630235900
    #
    # key_list = ['KLINEMIN_16_HHI2009_{}'.format(time_str) for time_str in range(time_start, time_end, 1)]
    # minute_info_list = db.get_multi_value(key_list)
    # print(minute_info_list)
    # #
    ...
    print(db.get_single_value('KLINEMIN_36_AAPL_20201015011400'))