...
import asyncio
from common.common_method import get_log
run_flag = True
            ...
            if send_type == 'SYNC_INSTR_REQ':
                ...
                if event[1] & zmq.POLLIN:
                    ...
                    if rev_data.type == QuoteMsgType.SYNC_INSTR_RSP:
                        ...
                        #test_log.debug('json_rsp_data: ', json_rsp_data)
...
if __name__ == "__main__":
    ...
    asy.run_until_complete(future=start('SYNC_INSTR_REQ', codegenerate_dealer_address))  # 执行事件队列, 直到最后的一个事件被处理完毕后结束
    # SYNC_INSTR_REQ：合约的    QUERY_RECOVERYDATA_REQ：（证券）静态的   SYNC_BASIC_REQ：（期货）静态的   SNAPSHOT_REQ：快照的