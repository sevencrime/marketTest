...
class MarketRedisDBClient(RedisDb):
    def __init__(self, host, port):
        RedisDb.__init__(self, host, port)

    def get_trade_data(self, exchange, instr_code, start_time_stamp, end_time_stamp):
        key = 'ZS_quote:tradetick:{}_{}'.format(exchange, instr_code)
        value_list = self.GetZsetValueByScore(key, start_time_stamp, end_time_stamp)
        json_rsp_list = []
        for value in value_list:
            value = value[20:]   # 因数据前缀有纳秒级别时间戳，此处去掉
