...
class CheckZMQ(unittest.TestCase):
    ...
    def test_stock_01_QuoteSnapshot(self):  # 股票-快照
        ...
        for info in check_info:
            ...
            if productCode == 'TRST':                                   # 信托产品
            ...
            elif productCode == 'INDEX':     # 指数 # BUG:http://jira.eddid.com.cn:18080/browse/HQZX-585
            ...
            elif productCode == 'BOND':     # 债券
                pass

            elif productCode == 'WRNT':     # 权证
                if 'warrant' in json_info.keys():   # 涡轮
                    ...
                    callput = warrant.get("callputType")                        # 认购认沽标识, BUG:http://jira.eddid.com.cn:18080/browse/HQZX-446
                    ...
                    hedge_value = warrant['hedgeValue']                     # 对冲值, BUG:http://jira.eddid.com.cn:18080/browse/HQZX-446
                    warrentType = warrant['warrentType']                    # 涡轮类型， BUG:http://jira.eddid.com.cn:18080/browse/HQZX-446

                elif 'cbbc' in json_info.keys():   # 牛熊证
                    cbbc = json_info['cbbc']
                    preclose = cbbc["preclose"]
                ...
                elif 'inner' in json_info.keys():  # 界内证, BUG:http://jira.eddid.com.cn:18080/browse/HQZX-586
            ...
            # assert rFRatio == round((riseFall / int(preclose)) * 10000)     # BUG:http://jira.eddid.com.cn:18080/browse/HQZX-524
    ...
    def test_stock_02_QuoteOrderBookData(self):  # 股票-盘口
        if self.check_json_list is None:
    ...
    def test_stock_03_QuoteBasicInfo(self, dealer_basic_info_list=None, dealer_record_time=None):  # 推送的静态数据
        ...
        for info in check_info:
            ...
            if type not in ['GREY_MARKET', 'BOND_DEBT_SECURITY']:
                trade_currency = stock['tradeCurrency']

            # 股票静态数据
            if exchange == 'SEHK' and type != 'GREY_MARKET':  # 港股 且 非暗盘
                casflag = stock['casflag']
                vcmflag = stock['vcmflag']

            #仅对港股供股权有效; 涡轮、牛熊证、界内证有效，已经终止交易的不需要判断,因BUG,注释掉
            # if exchange == 'SEHK' and type in ['EQUITY_PREFERENCE_SHARES','EQUITY_DEPOSITORY_RECEIPT_ORDINARY_SHARES',
            #              'EQUITY_DEPOSITORY_RECEIPT_PREFERENCE_SHARES',
            #              'WARRANT_EQUITY_WARRANT', 'WARRANT_DERIVATIVE_WARRANT',
            #              'WARRANT_EQUITY_LINKED_INSTRUMENT', 'WARRANT_CALLABLE_BULL_BEAR_CONTRACT',
            #              'OTHERS_NONE_OF_THE_ABOVE', 'WARRANT_INLINE_WARRANT'] and (instrCode not in ['52929','52531','48140','40371','20268','52665']):
            #     lasttradeday = stock['lasttradeday']
            #因BUG,注释掉
            if exchange == 'SEHK' and (productCode not in ['EQTY', 'TRST','WRNT', 'BOND']):  # 非股票，才检测 到期日 && 回收价，因停止交易的牛熊证，缺少这个字段，才不检查 2020.11.20
            ...
            # if exchange == 'SEHK' and type in ['TRUST_EXCHANGE_TRADED_FUND', 'TRUST_OTHER_UNIT_TRUSTS', 'TRUST_LEVERAGED_AND_INVERSE_PRODUCT']:
            #     release_day = stock['release_day']  # 发布日期(基金有效)(港股)

            if exchange == 'SEHK' and type != 'GREY_MARKET':
            ...
            # if type not in ['GREY_MARKET', 'WARRANT_DERIVATIVE_WARRANT'] and (instrCode not in ['52929','48140','40371','20268','52665']):  # 非暗盘、#港股、美股都需要
    ...
    def test_stock_03_01_QuoteBasicInfo(self):  # dealer模式下请求返回的静态数据
        ...
        for info in check_info:
            ...
            # self.test_stock_03_QuoteBasicInfo(info)
...
# -----------------------------------------------------------------------------------------------
    def test_stock_05_InstrumentInfo(self):
        ...
        for info in check_info:
            ...
            for instrument in instruments:
                ...
                #     if exchange in ['HKFE']:  # 港股需要检查中文名称，2020.11.19
                ...
                if categoryType == 'OPT':  # 期权合约特定信息
                    ...
                    if exchange == 'SEHK':
                        ...
                        if 'status' in instrument:  # 上市股票
                            producttype = stock['producttype']
                        else:  # 未上市股票 无producttype字段
                            self.assertNotIn(producttype, ['producttype'])
                        # 临时注销，待股票的状态正确后再判断
                        # if 'TRST' == instrumentCode:
                        #     self.assertTrue(producttype in ['TRUST_EXCHANGE_TRADED_FUND',
                        #                                     'TRUST_REAL_ESTATE_INVESTMENT_TRUST',
                        #                                     'TRUST_OTHER_UNIT_TRUSTS',
                        #                                     'TRUST_LEVERAGED_AND_INVERSE_PRODUCT'])
                        # else:
                        ...
                        #                                     'BOND_DEBT_SECURITY',
                        #                                     'OTHERS_NONE_OF_THE_ABOVE',
                        #                                     'WARRANT_INLINE_WARRANT'])
    ...
    def test_stock_06_PushKLineMinData(self):
        ...
        for info in check_info:
            ...
            if data.get("open") and data.get("high") and data.get("low"):
                ...
                # average = int(data['average'])  # BUG : http://jira.eddid.com.cn:18080/browse/HQZX-456
                ...
                settlementPrice = int(data['settlementPrice'])  # 昨收价, BUG:http://jira.eddid.com.cn:18080/browse/HQZX-457
                ...
                #######################################################################
                # 涨跌额 = 当前价格 - 结算价
                # self.assertTrue(riseFall == int(close - settlementPrice))    # BUG:昨收价精度和收盘价精度不一致
    ...
    def test_stock_07_PushKLineData(self):
        ...
        for info in check_info:
            ...
            if peroidTypeInt in [KLinePeriodType.DAY, KLinePeriodType.WEEK, KLinePeriodType.MONTH,
                ...
                settlementPrice = kData['settlementPrice']      # 昨收价
                ...
                if KLineKey == assert_json['{}_{}_current_time'.format(code, peroidType)]:
                    ...
                    if peroidTypeInt not in [KLinePeriodType.DAY, KLinePeriodType.WEEK, KLinePeriodType.MONTH,
                        ...
                        if updateDateTime in tradeTimelist:     # 同一天
                            ...
                            # self.assertTrue(currVol == vol - assert_json['{}_{}_last_vol'.format(code, peroidType)])  # BUG
                        else:   # 更新交易日
                            tradeTimelist = self.common.check_trade_status(exchange, code, curTime=updateDateTime, isgetTime=True, peroidType=peroidType)
    ...
    def test_stock_10_PushBrokerSnapshot(self):
        # 推送经纪席位数据
        if self.check_json_list == None:
            check_info = self.sq.get_pub_json_records(QuoteMsgType.PUSH_BROKER_SNAPSHOT, 1000)
        else:
            check_info = self.check_json_list
        self.assertTrue(check_info.__len__() >= 1)
        self.logger.debug('{} items to check!'.format(check_info.__len__()))
            ...
            for i in range(queue_sell.__len__()):
                ...
                if 'brokes' in sell_info:  # 部分买盘/卖盘无经济席位
                    for brokes in sell_info['brokes']:
                        ...
                        #经济席位编码只能是4位
                        self.assertTrue(len(broker_code) == 4)
